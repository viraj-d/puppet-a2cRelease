...
GO
PRINT N'Altering [DBO].[usp_CheckIncomingSequenceAlreadyUsed]...';


GO
ALTER PROCEDURE DBO.[usp_CheckIncomingSequenceAlreadyUsed]
	@Sequence BIGINT, 
	@SchoolId INT,
	@AwardingOrganisationCentreId INT
AS
	IF EXISTS
		(select 1 from AwardingOrganisationCentres where A2CSchoolId = @SchoolId and AwardingOrganisationCentreId = @AwardingOrganisationCentreId
		and @Sequence <= IncomingSequence)
		OR
	EXISTS
		(select 1 from OutSequence where AwardingOrganisationCentreId = @AwardingOrganisationCentreId and @Sequence = SequenceNumber)
		SELECT 1
	ELSE
		SELECT 0
GO
...
GO
ALTER PROCEDURE usp_ProcessEditIncomingSequenceNumber
	@SchoolId int,
	@awardingOrganisationCentreId int,
	@NewSequence bigint,
...
AS
	...
	BEGIN TRY
		...
		END
		ELSE --STEP 2 - mark pending out of sequence messages for processing, must be processed before any new incoming message after this update
		BEGIN
			...
			IF EXISTS (select 
						...
						where 
							...
							and m.AwardingOrganisationCentreId=@awardingOrganisationCentreId
							and m.schoolid = @SchoolId
							and isnull(m.ImportPriority, 9999) <> 9999)
			BEGIN
				select 
					@NewImportPriority = max(isnull(m.ImportPriority, 0)) + 1
				from 
					A2CTransactionResponseEnvelopeMessages m
				where 
					m.DataImportStatus = 0
...
GO
EXECUTE sp_updateextendedproperty @name = N'Version', @value = N'15.1.10.5';
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Import Received XML file from AO', 
		...
		ORDER BY 
			AT.SchoolId, AT.AWARDINGORGANISATIONCENTREID, AT.ImportPriority, AT.INCOMINGSEQUENCE
