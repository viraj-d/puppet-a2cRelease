...
function DeleteUser() {
    ...
    if (selectedUserIds.length > 0) {
        ...
        showConfirmationMessage(UserDeleteConfirmation, function () {
            busyDiv();
            postJSON('/User/DeleteUsers', { userIds: selectedUserIds }, function (data) {
                if (data.result) {
...
function UnLockUser() {
    ...
    if (selectedUserIds.length > 0) {
        ...
        else
        {
            selectedUserIds = selectedUserIds.substring(0, selectedUserIds.length - 1);
            busyDiv();
            postJSON('/User/UnlockUsers', { userIds: selectedUserIds }, function (data) {
                    if (data.result) {
...
function DeactivateUser() {
    ...
    if (selectedUserIds.length > 0) {
        ...
        if (AnyActive == false) {
            ...
            selectedUserIds = selectedUserIds.substring(0, selectedUserIds.length - 1);
             busyDiv();
            postJSON('/User/DeactivateUsers', { userIds: selectedUserIds }, function (data) {
                if (data.result) {
...
function ActivateUser() {
    ...
    if (selectedUserIds.length > 0) {
        ...
        if (AnyDeactive == false) {
            ...
            selectedUserIds = selectedUserIds.substring(0, selectedUserIds.length - 1);
                             busyDiv();
            postJSON('/User/ActivateUsers', { userIds: selectedUserIds }, function (data) {
                if (data.result) {
        ...
        else
        {
            ...
            if (rows && rows.length > 0) {
                if (rows[0].Status == 'Deactivated') {
                    ...
                    window.location.href = jQuery.validator.format('/User/Edit/{0}', rows[0].UserId);
                }
            }
            }
            else
...
function EditSchoolUser()
{
    ...
    postJSON('/User/Edit', schoolUser, function (result) {
        if (result && result.status) {
            window.location.href = '/User/Index/2';
        }
        else {
...
function AddSchoolUser()
{
    ...
    var schoolUser = {
        ...
        Surname: $('#Surname').val(),
        Email: $('#Email').val().trim(),
        Password: isSMTPConfigured == "False" ? $('#Password').val() : null,
        ConfirmPassword: isSMTPConfigured == "False" ? $('#ConfirmPassword').val() : null
    };
                busyDiv();

    postJSON('/User/Add', { schoolUser: schoolUser, isSmtpConfigured: isSMTPConfigured }, function (result) {
        if (result && result.status)
        {
